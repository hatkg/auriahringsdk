...
public class BleSDK {
    ...
    public static void DataParsingWithData(byte[] value, final DataListener2301 dataListener) {
        ...
        switch (value[0]) {
            ...
            case DeviceConst.Oxygen_data:
                ...
                break;
            case DeviceConst.Manual_Oxygen_data:
                // Process manual SpO2 data using getBloodoxygen method
    ...
    /**
     * Request manual SpO2 data
     * @param dateOfLastData last data date (format: yyyy.MM.dd HH:mm:ss)
     * @return command bytes
     */
    public static byte[] Manual_Oxygen_data(String dateOfLastData) {
        byte[] value = new byte[16];
        value[0] = DeviceConst.Manual_Oxygen_data;
        value[1] = (byte) 0x00; // Request manual data
        insertDateValue(value, dateOfLastData);
        crcValue(value);
        return value;
    }
