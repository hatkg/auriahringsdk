        ...
        if (!isinScan && isBleEnable()) {
            ...
            if (devices.size() > 0) {
                for (BluetoothDevice bluetoothDevice : devices) {
                    if (Canctians(bluetoothDevice.getName() != null ? bluetoothDevice.getName().toLowerCase() : "")) {
                        Device device = new Device();
                        device.setBluetoothDevice(bluetoothDevice);
                        device.setIsconted(false);
                        device.setPaired(true);
                        device.setIsdfu(bluetoothDevice.getName() != null && bluetoothDevice.getName().toLowerCase().contains("dfu"));
